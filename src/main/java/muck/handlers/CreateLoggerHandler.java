...
public class CreateLoggerHandler implements Handler {
    private static final Logger LOGGER = Logger.getLogger(CreateLoggerHandler.class.getName());

    private final BobClient bobClient;

    public CreateLoggerHandler(BobClient bobClient) {
    ...
    public void handle(ServerRequest req, ServerResponse res) {
        try {
            ...
            LOGGER.log(Level.INFO, "Creating logger: {0}", name);

            var success = bobClient.createLogger(body);

            if (success) {
                res.status(Status.OK_200);
                res.send("Logger created");
            } else {
                res.status(Status.INTERNAL_SERVER_ERROR_500);
                res.send("Failed to create logger");
            }
        } catch (Exception e) {
